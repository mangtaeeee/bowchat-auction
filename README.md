...
- **세션 관리와 인증 토큰 캐싱**  
  ...
  DB 접근보다 훨씬 빠른 속도의 In-memory 캐시가 필요했습니다.  
  Redis를 세션 스토리지 및 RefreshToken 저장소로 사용해  
  로그인 유지 및 토큰 재발급의 응답 지연을 최소화했습니다.  
- **Pub/Sub vs Kafka 역할 분리**  
  대규모 트래픽 구간에서는 Kafka Producer에 부하가 집중될 수 있어,  
  Redis를 **단기 버퍼(Front Queue)** 로 두어 요청을 임시 큐잉한 뒤 Kafka로 전달했습니다.  
  이를 통해 순간적인 트래픽 스파이크를 흡수하고,  
  Kafka는 영구 저장과 스트리밍 처리를 담당하도록 역할을 명확히 분리했습니다.  
  → 결과적으로 트래픽 급등 시에도 세션 안정성과 메시지 정합성을 동시에 확보했습니다.
---
