...
Client
  ...
  └── chat-service    (8084) → WebSocket 채팅 (예정)

Kafka (Event Bus)
  ├── user.created   → product/auction/chat-service 수신
  └── auction-bid    → chat-service 수신 (브로드캐스트)
...
PostgreSQL (스키마 분리)
  ...
  └── chat_service    → chat_rooms, chat_messages, user_snapshots (예정)
```
...
- 경매 입찰 이벤트 실시간 브로드캐스트

### 경매
- 입찰 이벤트 처리 및 실시간 가격 반영
- 낙관적 락(@Version)으로 동시 입찰 처리
- Kafka auction-bid 토픽으로 chat-service에 브로드캐스트
