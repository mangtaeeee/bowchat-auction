...
### 3. 사용자 인증은 user-service JWT를 기준으로 정리

MSA로 분리한 뒤에는 클라이언트가 `userId`를 직접 보내는 구조를 줄이고, 인증된 사용자 정보는 user-service가 발급한 JWT 클레임에서만 꺼내도록 맞췄다.
각 서비스가 인증 때마다 다시 user-service를 조회하면 의존성이 너무 커지고, 인증 경로가 네트워크 상태에 흔들리기 때문이다. 그래서 `userId`, `email`, `nickname`, `role`을 토큰에 담고, 다른 서비스는 그 클레임만으로 인증 컨텍스트를 만들도록 정리했다.

이렇게 해야 요청 바디의 `userId` 위변조를 막을 수 있고, 서비스가 나뉜 뒤에도 인증 책임은 user-service 한 곳에 남겨 둘 수 있다.

관련 코드:
- [user-service JwtProvider](./user-service/src/main/java/com/example/userservice/auth/jwt/JwtProvider.java)
- [product-service JwtProvider](./product-service/src/main/java/com/example/productservice/auth/JwtProvider.java)
- [auction-service JwtProvider](./auction-service/src/main/java/com/example/auctionservice/auth/JwtProvider.java)
- [chat-service JwtProvider](./chat-service/src/main/java/com/example/chatservice/auth/JwtProvider.java)
...
처음에는 내부 API를 빠르게 잠그기 위해 Docker Network와 `X-Service-Token` 조합을 썼다. 구현은 단순했지만, 이 방식만으로는 어떤 서비스가 호출했는지 구분하기 어렵고 권한도 서비스별로 나누기 힘들었다.

그래서 내부 호출은 Keycloak 기반 OAuth 2.0 client credentials로 맞췄다. 서비스마다 자기 client id와 secret으로 access token을 발급받고, 받는 쪽은 issuer와 scope를 검증하는 방식이다. 이렇게 해야 내부 호출도 사용자 인증과 분리된 별도 체계로 가져갈 수 있고, 이후 mTLS나 service mesh 같은 운영 단계 보안으로 확장하기도 수월하다.

실제 적용은 `auction-service`, `product-service`, `user-service`, `chat-service` 모두 같은 방향으로 맞췄고, 전환 안정성을 위해 기존 `X-Service-Token`도 병행 허용하고 있다. 즉 운영 기준은 OAuth 2.0 Bearer 토큰 검증으로 옮겼고, 기존 내부 토큰은 호환 레이어로만 남겨 둔 상태다.
