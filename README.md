...
| auction-service | 경매 시작/입찰, 낙관적 락, Outbox 기반 입찰 이벤트 발행 | [README](./auction-service/README.md) |
...
PostgreSQL (스키마 분리)
  ...
  |- auction_service -> auctions, auction_bids, outbox_events, user_snapshots
...
### 3. 사용자 인증은 Keycloak JWT를 기준으로 정리

현재는 `user-service 자체 JWT`를 서비스들이 공유하는 구조가 아니라, Keycloak이 발급한 access token을 각 서비스가 직접 검증하는 구조다.
로그인과 refresh token 재발급은 Keycloak이 담당하고, 각 서비스는 issuer/public key 기준으로 서명을 검증한 뒤 `userId`, `email`, `nickname`, `role` claim을 기존 principal 형태로 변환해 사용한다.

이렇게 해야:
- 토큰 발급 책임을 중앙화할 수 있고
- 서비스 간 shared secret 의존을 줄일 수 있으며
- 서버 간 내부 인증과 사용자 인증을 같은 IdP 기준으로 맞출 수 있다.

관련 코드:
- [user-service AuthService](./user-service/src/main/java/com/example/userservice/auth/service/AuthService.java)
- [user-service KeycloakAuthService](./user-service/src/main/java/com/example/userservice/auth/service/KeycloakAuthService.java)
- [user-service SecurityConfig](./user-service/src/main/java/com/example/userservice/config/SecurityConfig.java)
- [auction-service UserJwtAuthenticationConfig](./auction-service/src/main/java/com/example/auctionservice/auth/config/UserJwtAuthenticationConfig.java)
- [product-service UserJwtAuthenticationConfig](./product-service/src/main/java/com/example/productservice/auth/config/UserJwtAuthenticationConfig.java)
- [chat-service UserJwtAuthenticationConfig](./chat-service/src/main/java/com/example/chatservice/auth/config/UserJwtAuthenticationConfig.java)
- [chat-service JwtHandshakeInterceptor](./chat-service/src/main/java/com/example/chatservice/websocket/JwtHandshakeInterceptor.java)

### 4. 경매 입찰은 로컬 검증 + 낙관적 락 + Outbox로 처리
...
또한 입찰 저장과 Kafka 발행 사이의 정합성을 맞추기 위해, 입찰 저장과 outbox 저장을 같은 트랜잭션으로 묶고 스케줄러가 outbox를 읽어 `auction-bid` 이벤트를 발행하도록 바꿨다.
...
- [AuctionOutboxService](./auction-service/src/main/java/com/example/auctionservice/outbox/AuctionOutboxService.java)
- [OutboxRelayScheduler](./auction-service/src/main/java/com/example/auctionservice/outbox/OutboxRelayScheduler.java)
...
### 6. 내부 API 인증은 Keycloak 기반 OAuth 2.0으로 공통화

처음에는 내부 API를 빠르게 잠그기 위해 Docker Network와 `X-Service-Token` 조합을 썼다. 구현은 단순했지만, 어떤 서비스가 호출했는지 구분하기 어렵고 권한을 서비스별로 세분화하기도 어려웠다.

그래서 지금은 내부 호출을 Keycloak 기반 OAuth 2.0 `client_credentials`로 통일했다. 각 서비스는 자기 `client_id` / `client_secret`으로 access token을 발급받고, 받는 쪽은 issuer와 scope 또는 role을 검증한다.

현재 적용 상태:
- `INTERNAL_SECRET`, `X-Service-Token` fallback 제거
- `auction-service`, `product-service`, `chat-service`는 Feign interceptor에서 Bearer Token 자동 주입
- `user-service`의 `/internal/**`는 Keycloak issuer + scope/role 기준으로만 허용

관련 코드:
- [auction-service SecurityConfig](./auction-service/src/main/java/com/example/auctionservice/auth/config/SecurityConfig.java)
- [product-service SecurityConfig](./product-service/src/main/java/com/example/productservice/auth/config/SecurityConfig.java)
- [user-service SecurityConfig](./user-service/src/main/java/com/example/userservice/config/SecurityConfig.java)
- [chat-service SecurityConfig](./chat-service/src/main/java/com/example/chatservice/auth/config/SecurityConfig.java)
- [auction-service OAuth2ClientConfig](./auction-service/src/main/java/com/example/auctionservice/auth/config/OAuth2ClientConfig.java)
- [product-service OAuth2ClientConfig](./product-service/src/main/java/com/example/productservice/auth/config/OAuth2ClientConfig.java)
- [chat-service OAuth2ClientConfig](./chat-service/src/main/java/com/example/chatservice/auth/config/OAuth2ClientConfig.java)
- [infra Keycloak realm import](./infra/keycloak/import/bowchat-realm.json)
...
이 문서는 분리 작업 기록을 기준으로 작성됐기 때문에, 아래 인증 구조가 현재 운영 기준이다.
예전의 `user-service 자체 JWT` 또는 `X-Service-Token` 설명이 보이더라도 최신 기준은 이 섹션을 따른다.

### 현재 전체 인증 구조

- 사용자 로그인 인증: Keycloak
- 사용자 access token 발급: Keycloak
- 사용자 refresh token 발급/재발급: Keycloak
- 서버 간 내부 API 토큰 발급: Keycloak `client_credentials`
- 토큰 서명 검증: 각 서비스가 Keycloak issuer/public key로 검증
- 로그아웃 직후 access token 즉시 차단: Redis blacklist
- 회원가입/유저 프로필/도메인 데이터 저장: 각 서비스 도메인 책임

즉, 지금은 `토큰을 만드는 주체와 검증 기준은 Keycloak`, `도메인 데이터와 로그아웃 즉시 차단 보완은 애플리케이션` 구조다.
...
- `/auth/login`, `/auth/refresh`, `/auth/logout` API 제공
- refresh token 세션 관리
- access token blacklist 등록

#### product-service / auction-service / chat-service
- 일반 사용자 Bearer Token 검증
- Keycloak JWT를 기존 `UserPrincipal(userId, email, nickname, role)` 형태로 변환
- 도메인 인가 처리
- blacklist 체크
...
```text
사용자 -> user-service /auth/login
       -> user-service가 Keycloak 사용자 동기화
       -> user-service가 Keycloak token endpoint 호출
       -> Keycloak이 access token / refresh token 발급
       -> user-service가 refresh token 저장
       -> 클라이언트에 Keycloak access token 반환
```

### 로그아웃 시퀀스

```text
사용자 -> user-service /auth/logout
...
- 로그인 인증 서버
- 사용자/내부 서비스 토큰 발급 서버
- refresh token 재발급 기준점
- JWT 공개키/issuer 기준점
- role/scope 기반 인증 기준점

### 아직 남은 과도기

- Google/Kakao 소셜 로그인 경로도 Keycloak 브로커를 기준으로 동작한다.
- 운영에서는 Keycloak에 Google/Kakao identity provider를 실제 값으로 연결해야 한다.
- 일반 사용자 토큰을 안정적으로 해석하려면 Keycloak access token에 `userId`, `email`, `nickname`, `role` claim이 포함되도록 mapper를 맞춰야 한다.



### Keycloak 브로커 설정 메모

현재 social login 경로는 Keycloak broker를 전제로 한다.
필수 client/alias:
- browser client: `bowchat-web`
- direct grant client: `bowchat-login`
- admin client: `bowchat-admin`
- identity provider alias: `google`, `kakao`

관련 코드:
- [KeycloakAuthorizationRequestResolver](./user-service/src/main/java/com/example/userservice/auth/oauth/config/KeycloakAuthorizationRequestResolver.java)
- [KeycloakOidcUserService](./user-service/src/main/java/com/example/userservice/auth/oauth/service/KeycloakOidcUserService.java)
- [OAuth2SuccessHandler](./user-service/src/main/java/com/example/userservice/auth/oauth/handler/OAuth2SuccessHandler.java)
- [Keycloak realm import](./infra/keycloak/import/bowchat-realm.json)
