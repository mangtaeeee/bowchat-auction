...
Redis database 분리 대신 **키 Prefix 방식**으로 논리적으로 분리했다.
Redis Cluster 환경에서는 database가 0만 지원되므로 database 분리는 실무에서 사용하기 어렵다.
모든 서비스가 동일한 database(0)를 사용하고 키에 서비스별 prefix를 붙여서 충돌을 방지한다.

```
user-service    → refresh_token:{email}, blacklist:{token}
product-service → product:user:{userId}  (UserSnapshot 캐시)
auction-service → auction:user:{userId}  (UserSnapshot 캐시)
chat-service    → chat:user:{userId}     (UserSnapshot 캐시)
```

블랙리스트는 prefix 없이 전 서비스가 공유하므로 user-service에서 등록하면 product-service에서 바로 체크된다.
...
public UserSnapshot getUser(Long userId) {
    // 1. Redis 캐시
    Object cached = redisTemplate.opsForValue().get("product:user:" + userId);
    ...
    if (local != null) {
        redisTemplate.opsForValue().set("product:user:" + userId, local, TTL);
        return local;
    }
    ...
    redisTemplate.opsForValue().set("product:user:" + userId, snapshot, TTL);
    return snapshot;
}
```
